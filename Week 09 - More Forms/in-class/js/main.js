...
function loadMSGrid() {
    ...
    for (let i = 0; i < ROWS; i++) {
        ...
        for (let j = 0; j < COLS; j++) {
            ...
            // Add an event listener on cell right click.
            msCell.addEventListener("contextmenu", onCellRightClick);
...
function initialiseMSHeader() {
    const remainingMinesCell = document.getElementById("remaining-mines");
    remainingMinesCell.textContent = padString(N_MINES + "", "0", 3);
    // JS Hack: "" + whater casts to string
}
...
function onCellClick(event) {
    ...
    msCell.removeEventListener("click", onCellClick); // Remove the event listener
    removeEventListener("contextmenu", onCellRightClick);
...
/*
On removing event listeners

To remove an event listener from an element, we need to call `.removeEventListener()` with the very 
same arguments as `.addEventListener()` was called to add this listener. Otherwise it has either no
...
function expandCells(row, col) {
    ...
    */
    ...
    let neighbours, number, msCell, currentId; // Auxilliary variables
    while (frontier.length > 0) { // While there is stuff to explore...
        ...
        currentId = current.join("-");
        visited.add(currentId);
        msCell = document.getElementById(currentId);
        msCell.removeEventListener("click", onCellClick);
        msCell.removeEventListener("contextmenu", onCellRightClick);
        if (!msCell.classList.contains("visited")) {
            cellsVisible++;
        }
        number = GRID[current[0]][current[1]];
        msCell.classList.add("visited");
        if (msCell.firstChild)
            msCell.removeChild(msCell.firstChild);
        if (number === 0) {
...
function onCellRightClick(event) {
    // `.preventDefault()` prohibits the browser from executing the default action on this event.
    event.preventDefault();
    // console.log(event);
    const msCell = event.currentTarget;
    /* event.target corresponds to the element that triggered
    * the event.
    * event.currentTarget corresponds to the closest (grand?)parent that has the event listener attached on it.
    */
    const remainingMinesElement = document.getElementById("remaining-mines");
    if (msCell.children.length === 0 && remainingMines > 0) {
        const flagImgTag = document.createElement("img");
        flagImgTag.src = "../assets/icons/flag.svg";
        flagImgTag.alt = "Mine flag icon"; // Alternative text in case image does not load.
        // flagImgTag.style.width = "20px"; // Typically a bad idea.
        msCell.appendChild(flagImgTag);
        msCell.removeEventListener("click", onCellClick);
        remainingMines--;
        remainingMinesElement.textContent = padString(remainingMines + "", 0, 3);
    } else if (msCell.children.length > 0) {
        msCell.removeChild(msCell.firstChild);
        msCell.addEventListener("click", onCellClick);
        remainingMines++;
        remainingMinesElement.textContent = padString(remainingMines + "", 0, 3);
...
function startTimer() {
    ...
    function tickTimer() {
        setTimeout(() => {
            const time = +timerSpan.textContent;
            timerSpan.textContent = padString(time + 1 + "", 0, 3);
            tickTimer();
        }, 1000);
    }
    tickTimer();
...
function createModal(text) {
    ...
    modalContainer.textContent = text; // Best practice, compared to `.innerText`
    const restartButton = createButton("Restart!", resetGame);
    modalContainer.appendChild(restartButton);
    // `setTimeout()` ensures that the code below will be executed at a different frame, after
    // this function, `createModal()` has been completely executed, first.
    // setTimeout(() => modalContainer.style.transform = "translateY(0px)", 0);
...
function createButton(buttonText, onClick) {
    ...
    button.onclick = onClick; // Equivalent to `button.addEventListener("click", onClick);`
    return button;
}

function resetGame() {
    // This returns a list, so we have to get its first (and unique, in this case) element
    const modalContainer = document.getElementsByClassName("modal-container")[0];
    modalContainer.classList.add("modal-container-shift-up");
    setTimeout(() => { location.reload(); }, 190); // HACK: Dummy, fix later!
}
...
function hasWon() {
    return ROWS * COLS - N_MINES == cellsVisible;
}

function padString(string, pad, length) {
    // Assuming string.length <= length
